...
/*
 - 추상메소드만으로 이루어짐
 - 클래스 바디가 없고 메소드의 프로토타입 만 선언되어 있으면, 추상메소드임
 - 추상메소드를 사용하는 이유는 앞으로 추가되거나 구현되어야 하는
        기능의 설계 역활을 하여 실제 기능은 구현하지 않고
        메소드 프로토타입만 구현하는 것을 말함
 - 추상메소드는 건축물에서 구조를 이루는 철근과 같고 설계도 와 같은 역활을 함
 ...
 - 인터페이스상에 있는 메소드를 하나라도 구현하지 않으면, 인터페이스를 
        상속받은 클래스는 추상 클래스가 됨
 - 인터페이스에 선언된 메소드르 호출하면, 그 인터페이스를 구현한 
     클래스의 메소드가 자동으로 호출됨.
...
* */
  /*
   인터페이스 구조
     ...
     - 각 클래스의 멤버변수에 접근하는 경우는 그 클래스 타입으로 형변환 을 
             해주어야 함.
